...
break;}}
$.fn.animateRotate=function(angle,duration,easing,complete){return this.each(function(){var $elem=$(this);$({deg:0}).animate({deg:angle},{duration:duration,easing:easing,step:function(now){$elem.css({transform:'rotate('+now+'deg)'});},complete:complete||$.noop});});};function isDescendant(parent,child){if(parent==null)return false;var node=child;while(node!=null){if(node==parent){return true;}
...
if(settings.trigger=="hover"){button.hover(function(){if(!el.hasClass("active"))
{el.showIcon(button,settings);}});el.bind({mouseleave:function(e){var evt=e||window.event;if(!isDescendant(button[0],evt.relatedTarget)&&!isDescendant(this,evt.relatedTarget))
{el.hideIcon(button,settings);}}});}else{button.click(function(){if(el.css('visibility')=="visible"){el.hideIcon(button,settings);}else{el.showIcon(button,settings);}});}});}}(window.jQuery);